...
export const STATE_STYLE: Record<ProductState, { chip: string; dot: string; text: string }> = {
  active: { chip: 'bg-brand/[0.12] text-brand-ink', dot: 'bg-brand', text: 'text-brand-ink' },
  need_order: { chip: 'bg-warn/[0.14] text-warn-ink', dot: 'bg-warn', text: 'text-warn-ink' },
  out: { chip: 'bg-danger/[0.12] text-danger-ink', dot: 'bg-danger', text: 'text-danger-ink' },
